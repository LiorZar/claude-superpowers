...
Subagent (general-purpose):
  ...
  prompt: |
    ...
    **Head:** [HEAD_SHA]
    ...
    Your review is read-only on this checkout. Do not mutate the working tree, the index, HEAD, or branch state in any way. Use tools like `git show`, `git diff`, and `git log` to inspect history. If you need a working copy of a different revision, clone into a separate temporary directory and check out the revision there (e.g. `git clone . /tmp/review-[SHA] && git -C /tmp/review-[SHA] checkout [SHA]`) — never move HEAD on this checkout.
