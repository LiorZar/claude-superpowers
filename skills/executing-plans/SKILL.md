...
digraph process {
    ...
    "Setup: confirm git workflow, workspace + ledger, read plan + spec, pre-flight scan" [shape=box];
    ...
    "Setup: confirm git workflow, workspace + ledger, read plan + spec, pre-flight scan" -> "task-start: brief + BASE; read the brief";
...
Work happens in place on the current branch — there is no worktree to set
up — but the branch and commit approach still need your human partner's
...
- Honor any git preference your human partner already stated in their
  instructions — do not re-ask.
- **Never** start implementation on a main/master branch without explicit
  consent. If the current branch is `main`/`master` and they did not pick
  option 1, confirm before proceeding.
- Remember the chosen cadence — the plan's commit steps run according to it.
  Under "one commit at the end," the per-task commit steps are deferred and
  that single commit is made before the final review.
...
[Setup: git workflow confirmed — new branch, commit after each task]
