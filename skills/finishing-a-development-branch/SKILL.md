...
**Work happens in place on the current branch — there is no separate
worktree or workspace to detect or clean up.** The only thing that changes
the menu is whether HEAD is on a named branch:
...
| State | Menu |
|-------|------|
| Named branch (the normal case) | Standard 3 options |
| Detached HEAD | Reduced 2 options (no merge) |
...
**Named branch (the normal case) — present exactly these 3 options:**
...
Wait for that exact confirmation. When it arrives, switch to <base-branch>
and force-delete the branch:
