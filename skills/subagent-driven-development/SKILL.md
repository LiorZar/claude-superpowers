...
digraph process {
    ...
    "Setup: confirm git workflow, ledger check, read plan, pre-flight review" [shape=box];
    ...
    "Setup: confirm git workflow, ledger check, read plan, pre-flight review" -> "Dispatch implementer subagent (./implementer-prompt.md)";
...
Work happens in place on the current branch, under the git workflow you
just confirmed. Never start implementation on a main/master branch
