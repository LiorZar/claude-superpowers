...
**Design spec:** `docs/specs/2026-07-15-sdd-fix-loop-redesign-design.md`. Read it before starting any task.
...
- Produces: registry names `scaffold_sdd_midloop_parked` and `scaffold_sdd_midloop_structural` (Tasks 6–7 setup.sh call these); fixture repo with `docs/plans/metrics-plan.md`, Tasks 1–2 implemented and committed, `.superpowers/sdd/progress.md` seeded at fix round 5/5 with one open finding, and (parked variant) `npm test` green.
  ...
  test('scaffoldSddMidloopStructural seeds a plan-contradiction finding', () => {
    ...
    try {
      ...
      const plan = readFileSync(
        join(dir, 'docs/plans/metrics-plan.md'),
        'utf8',
      );
...
function scaffoldSddMidloop(ctx: HelperContext, opts: MidloopOptions): void {
  ...
  const ledger = [
    '# SDD Progress Ledger',
    'Plan: docs/plans/metrics-plan.md',
...
pre() {
    ...
    file-contains 'docs/plans/report-plan.md' 'asserts nothing'
}
...
at docs/plans/metrics-plan.md, mid-execution: Task 1 is
...
docs/plans/metrics-plan.md with the
...
You are resuming an interrupted SDD session. The repo has a three-task plan
at docs/plans/metrics-plan.md, mid-execution: Task 1 complete,
...
`evals/docs/skills/triaging-a-failing-eval.md` before touching
